...
train_X_raw, train_ner, train_id, dev_X_raw, dev_ner, valid_id = data_manager.parseData(file_name=file_namne,valid_num=10000)
assert len(train_X_raw) == len(train_ner)
## deal for bert
train_X = [['[CLS]']+list(temp)+['[SEP]'] for temp in train_X_raw]
dev_X = [['[CLS]']+list(temp)+['[SEP]'] for temp in dev_X_raw]
