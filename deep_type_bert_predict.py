...
data_all = data_manager.deep_type_predict(filename='result/el_dev_result(1).csv')
...
for round in range(5):
    ...
    seed_torch(2019)
    dev_X = [['[CLS]']+list(temp[1])+['[SEP]'] for temp in data_all]
    dev_pos = [[x[2][0]+1, x[2][1]+1]for x in data_all]
    ...
    valid_dataset = SPO_BERT_LINK(dev_X, t, pos=dev_pos, type=None)
