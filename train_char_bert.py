...
import torch, os
from spo_dataset import SPO, get_mask, collate_fn_withchar, collate_fn, SPO_BERT
from spo_model import SPOModel, SPO_Model_Simple, SPO_Model_Bert
...
seed_torch(2019)
train_X, train_pos, train_label, train_ner, dev_X, dev_pos, dev_label, dev_ner = read_data(char=True)
...
train_dataset = SPO_BERT(train_X, train_pos, t, pos_t, label=train_label, combined_char_t=combined_char_t)
valid_dataset = SPO_BERT(dev_X, dev_pos, t, pos_t, label=dev_label, combined_char_t=combined_char_t)
...
model = SPO_Model_Bert(encoder_size=128, dropout=0.5,
                 seq_dropout=0.0,
...
for epoch in range(25):
    model.train()
    train_loss = 0
    for index, X, pos_tags, length, numerical_features, label in tqdm(train_dataloader):
